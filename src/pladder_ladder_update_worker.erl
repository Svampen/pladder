...
%%%
%%% @end
%%% Created : 18. Jul 2017 15:27
%%%-------------------------------------------------------------------
-module(pladder_ladder_update_worker).
...
start_link() ->
    PoolSize = application:get_env(pladder, workers, 100),
    WPoolConfigOpts = application:get_env(pladder, wpool_opts, []),
    WPoolOptions = [
    ...
    wpool:start_pool(?SERVER, WPoolConfigOpts ++ WPoolOptions).

update_entry(Entry, Ladder, ReplyTo) ->
    wpool:cast(?SERVER, {update_entry, Entry, Ladder, ReplyTo}).
